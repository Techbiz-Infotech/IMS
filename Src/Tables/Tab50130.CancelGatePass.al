table 50130 "Cancel Gatepass"
{
    Caption = 'Cancel Gatepass';
    DataClassification = ToBeClassified;
    fields
    {
        field(1; "Cancel Gatepass No."; Code[20])
        {
            Caption = 'Cancel Gatepass No.';
            DataClassification = ToBeClassified;
            ...
            begin
                if "Cancel Gatepass No." <> xRec."Cancel GatePass No." then begin
                    ImsSetup.Get;
                    NoSeriesMgmt.TestManual(ImsSetup."Cancel Gatepass Nos");
                    "NO.Series" := '';
                end;
            end;
        ...
        field(10; "Cancel GatePass Status"; Enum "Active/In-Active Enum")
        {
            Caption = 'Cancel GatePass Status';
            DataClassification = ToBeClassified;
            Editable = false;
        }
        ...
        field(46; "Cancel GatePass Printed"; Boolean)
        {
            Caption = 'Cancel GatePass Printed';
            DataClassification = ToBeClassified;
            Editable = false;
        }
    ...
    procedure AssistEdit(GatePassRec: Record "Cancel GatePass"): Boolean
    var
    ...
    begin
        ...
        if ManifestLine.FindFirst() then begin
            ManifestLine.Released := false;
            ManifestLine."Date Released" := 0D;
            ManifestLine.Modify();
        end;
