report 50052 "Monthly TEU Summary report"
{
    ...
    local procedure GetContainerWeight(): Decimal
    begin
        if ManifestLine."Shortcut Dimension 5 Code" = '40FT' then
            exit(2);
        exit(1);
    end;
    ...
    var
        MonthTxt: Text[7];
    begin
        ManifestLine.SetRange("Date Released", StartDate, EndDate);
        if ManifestLine.FindSet() then
            repeat
                MonthTxt := Format(ManifestLine."Date Released", 0, '<Year4>-<Month,2>');
                AddToCustomerMatrix(
                    ManifestLine."Consignee Name",
                    MonthTxt,
                    GetContainerWeight(),
                    ManifestLine."Shortcut Dimension 3 Code");
