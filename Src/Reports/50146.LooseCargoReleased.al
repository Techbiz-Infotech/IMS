report 50146 "Loose Cargo Released"
{
    ApplicationArea = All;
    Caption = 'Loose Cargo Released';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where("Shortcut Dimension 4 Code" = filter('LOOSE'), Released = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Loose Cargo Released', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
    ...
    begin
        TempExcelBuffer.CreateNewBook('Loose Cargo Released Report');
        TempExcelBuffer.WriteSheet('Loose Cargo Released Report', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('Loose Cargo Released Report');
