report 50150 "Loose Cargo Stock"
{
    ApplicationArea = All;
    Caption = 'Loose Cargo Stock';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where("Shortcut Dimension 4 Code" = filter('LOOSE'), Received = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Loose Cargo Stock As on' + '  ' + Format(EndDate), false, '', true, false, true, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
        ...
        TempExcelBuffer.AddColumn('In Operation', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Empty Depot Name', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.AddColumn('Guarantee From Received', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
    ...
    begin
        TempExcelBuffer.CreateNewBook('Loose Cargo Stock Status');
        TempExcelBuffer.WriteSheet('Loose Cargo Stock Status', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('Loose Cargo Stock Status');
